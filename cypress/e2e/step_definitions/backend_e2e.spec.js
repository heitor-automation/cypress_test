...
When("the user send the request to POSTcreatUser", () => {
  cy.api({
    method: "POST",
    url: `${Cypress.env("baseUrlBack")}/usuarios`,
    body: datBody,
...
When("the user send the request to PUTupdatUser", () => {
  cy.api({
    method: "PUT",
    url: `${Cypress.env("baseUrlBack")}/usuarios/${userId}`,
    body: datBody,
    failOnStatusCode: false,
  }).then((response) => {
    apiResponse = response
...
Then("the user will validate the api response according to the scenario {string} {string}", (type, field) => {
    cy.wrap(apiResponse).should((aresp) => {
      ...
      if (type === "positive") {
        expect(aresp.body.message).to.eq(data.validationMessages.user.userCreated)
      } else {
...
Then("the user will validate the api update response according to the scenario {string}", (type) => {
    cy.wrap(apiResponse).should((aresp) => {
      ...
      if (type === "positive") {
        expect(aresp.body.message).to.eq(data.validationMessages.user.userupedated)
      }
    })
  }
)
...
Then("the user will validate the api delete response according to the scenario {string}", (type) => {
    cy.wrap(apiResponse).should((resp) => {
      ...
      if (type === "negative") {
        expect(resp.body.message).to.have.string(data.validationMessages.cart.cartAssociated)
      } else {
        expect(resp.body.message).to.be.oneOf([
          data.validationMessages.user.userNotDeleted,
          data.validationMessages.user.userDeleted,
        ])
      }
    })
  }
